...
set "_SOURCE_MAIN_DIR=%_SOURCE_DIR%\main\dart"
...
@rem see https://dart.dev/guides/libraries/private-files
call :rmdir "%_ROOT_DIR%.dart_tool"
if exist "%_ROOT_DIR%pubspec.lock" del "%_ROOT_DIR%pubspec.lock"
goto :eof
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_DART_CMD%" analyze "%_SOURCE_MAIN_DIR%" %__DART_OPTS%
) else if %_VERBOSE%==1 echo Analyze Dart source files in directory "!_SOURCE_DIR:%_ROOT_DIR%\=!" 1>&2
)
call "%_DART_CMD%" analyze "%_SOURCE_MAIN_DIR%" %__DART_OPTS% %_STDOUT_REDIRECT%
if not %ERRORLEVEL%==0 (
...
set __SOURCE_FILES=
set __N=0
for /f "delims=" %%f in ('dir /s /b "%_SOURCE_MAIN_DIR%\main.dart" 2^>NUL') do (
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_DART_CMD%" pub upgrade 1>&2
) else if %_VERBOSE%==1 ( echo Resolve project dependencies 1>&2
)
call "%_DART_CMD%" pub upgrade
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to resolve project dependencies 1>&2
    set _EXITCODE=1
    goto :eof
)
set __DART_OPTS="--enable-experiment=macros" --output "%_EXE_FILE%"
if %_DEBUG%==1 set __DART_OPTS=--verbose %__DART_OPTS%

if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_DART_CMD%" compile exe %__DART_OPTS% %__SOURCE_FILES% 1>&2
) else if %_VERBOSE%==1 echo Compile %__N_FILES% into directory "!_TARGET_DIR:%_ROOT_DIR%=!" 1>&2
)
call "%_DART_CMD%" compile exe %__DART_OPTS% %__SOURCE_FILES% %_STDOUT_REDIRECT%
if not %ERRORLEVEL%==0 (
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_DART_CMD%" doc %__DARTDOC_OPTS% "%_SOURCE_MAIN_DIR%"
) else if %_VERBOSE%==1 echo Generate documentation into directory "!_TARGET_DOCS_DIR:%_ROOT_DIR%=!" 1>&2
)
call "%_DART_CMD%" doc %__DARTDOC_OPTS% "%_SOURCE_MAIN_DIR%"
if not %ERRORLEVEL%==0 (
